...
export default function csp(headers: Headers, options?: Options): void {
  options ??= defaults;
  ...
  if (
    typeof options["Report-To"] === "object" &&
    headers.get("content-type")?.includes("html")
  ) {
  ...
  if (
    ...
    const contentSecurityPolicy = stringifyContentSecurityPolicy(
